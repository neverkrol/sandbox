...
if [ "$1" == "--ansible" ];then
	echo ""
  	...
  	mkdir -p $ANSIBLE_DIR
        ...
        echo "[all:vars]" > $ANSIBLE_DIR/00_inventory.yml
        echo "ansible_connection=ssh" >> $ANSIBLE_DIR/00_inventory.yml
        echo "ansible_user=vagrant" >> $ANSIBLE_DIR/00_inventory.yml
        echo "[all]" >> $ANSIBLE_DIR/00_inventory.yml
        for i in $(docker ps -a --format "{{ .Names }}" |grep "vmparc" );do
          infos_conteneur=$(docker inspect -f '{{.NetworkSettings.IPAddress }}' ${i})
          echo "${infos_conteneur}" >> $ANSIBLE_DIR/00_inventory.yml
        done
